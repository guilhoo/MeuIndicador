...
const TodosPacientesScreen = () => {
  ...
  const [filteredPacientes, setFilteredPacientes] = useState([]); // Agentes filtrados


  // Função para buscar pacientes no Firebase
  const fetchPacientes = async () => {
    try {
      const snapshot = await firestore().collection('pacientes').get();
  ...
  const renderPaciente = ({ item }) => (
    <View style={styles.pacienteItem}>
      ...
      <Text style={styles.pacienteInfo}>Indicador: {item.indicador}</Text>
    </View>
  );
...
export default TodosPacientesScreen;

...
const GravidasListScreen = () => {
  ...
  const fetchPacientes = async () => {
    try {
      const snapshot = await firestore().collection('pacientes').where('indicador', '==', 'gravida').get();
  ...
  const renderPaciente = ({ item }) => (
    <View style={styles.pacienteItem}>
      ...
      <Text style={styles.pacienteInfo}>
        Microárea: {item.microarea ? item.microarea : "Não informado"}
      </Text>
    </View>
  );
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Lista de Grávidas</Text>
      </View>
...
export default GravidasListScreen;
